...
class Executor {
   public:
    ...
    Executor(const string &fname, bool debug_)
    ...
    bool cmd_write(Command c, vector<string> &lines, string &params,
    ...
    bool cmd_put_register(Command c, vector<string> &lines, int &dot,
                          bool &modified, char reg = '"', bool after = true);

    bool cmd_yank(Command c, vector<string> &lines, char reg = '"');
