...
int main() {
    vector<TestCase> tests = {
        ...
        {"1,5,7p",
         "addr1=5(NUMBER) addr2=7(NUMBER) command=p params=''",
         0,
         {}},

        // Regex addresses
        {"/foo/p",
         "addr1=0(REGEX_FWD:foo) addr2=0(NONE) command=p params=''",
         0,
         {}},
        {"?bar?d",
         "addr1=0(REGEX_BACK:bar) addr2=0(NONE) command=d params=''",
         0,
         {}},
        {"/foo/,/bar/d",
         "addr1=0(REGEX_FWD:foo) addr2=0(REGEX_FWD:bar) command=d params=''",
         0,
         {}},
        {"?baz?;5p",
         "addr1=0(REGEX_BACK:baz) addr2=5(NUMBER) command=p params=''",
         0,
         {}},
        {"/a\\/b/p",
         "addr1=0(REGEX_FWD:a\\/b) addr2=0(NONE) command=p params=''",
         0,
         {}},

        // Marks
        {"'a,'bz",
         "addr1=5(MARK:a) addr2=10(MARK:b) command=z params=''",
         0,
         {{"a", 5}, {"b", 10}}},
        {"'c",
         "addr1=35(MARK:c) addr2=0(NONE) command= params=''",
         0,
         {{"c", 35}, {"d", 0}}},

        // Commands with params
        {"w filename.txt",
         "addr1=0(NONE) addr2=0(NONE) command=w params='filename.txt'", 0},
        {"3,7s/old/new/g",
         "addr1=3(NUMBER) addr2=7(NUMBER) command=s params='/old/new/g'", 0},
        {"$a some text",
         "addr1=0(LAST) addr2=0(NONE) command=a params='some text'", 0},
        {"e otherfile",
         "addr1=0(NONE) addr2=0(NONE) command=e params='otherfile'", 0},
        {"E forcefile",
         "addr1=0(NONE) addr2=0(NONE) command=E params='forcefile'", 0},
        {"f myfile", "addr1=0(NONE) addr2=0(NONE) command=f params='myfile'",
         0},
        {"r !ls", "addr1=0(NONE) addr2=0(NONE) command=r params='!ls'", 0},
        {"wq out.txt",
         "addr1=0(NONE) addr2=0(NONE) command=w params='q out.txt'", 0},
        ...
        {"2!sort",
         "addr1=2(NUMBER) addr2=0(NONE) command=! params='sort'",
         0,
         {}},
        {"# comment",
         "addr1=0(NONE) addr2=0(NONE) command=# params='comment'",
         0,
         {}}};
    ...
    for (int i = 0; i < (int)tests.size(); i++) {
        ...
        Command cm =
            Command::build_command(c, a1, a2, params, t.dot, 5, t.marks);
        got << cm;
        cout << "Got:    " << got.str() << "\n";
